...
module.exports = class SSO {
  ...
  getSsoUrl (currentPath) {
    const now = new Date();
    const expires = new Date(now.getTime() + this._settings.cookie.ttl);
    const signer = Crypto.createSign('sha256');
    ...
    const query = Qs.stringify({
      ...
      returnto: `${this._settings.baseUrl}${currentPath}`,
      state: '',
      expires: expires.toISOString()
    });
    const url = `${this._settings.ssoUrl}?${query}`;
